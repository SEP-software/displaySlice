...
using namespace SEP;
partial_data_float::partial_data_float(const std::string& title,
                                       const std::string& nm,
...
std::shared_ptr<buffer> partial_data_float::create_buffer(
...
partial_data_byte::partial_data_byte(const std::string& title,
                                     const std::string& nm,
                                     std::shared_ptr<hypercube> g,
                                     std::shared_ptr<io_func> i,
                                     std::shared_ptr<paramObj> p, int in,
...
std::shared_ptr<buffer> partial_data_byte::create_buffer(
    std::shared_ptr<orient_cube> pos, int iax1, int iax2) {
  std::vector<int> nw(8, 1), fw(8, 1);
  std::vector<axis> axes = grid->getAxes();
  for (int i = axes.size(); i < 8; i++) axes.push_back(axis(1));
