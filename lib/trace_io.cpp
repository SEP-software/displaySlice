#include "trace_io.h"
using namespace SEP;
void trace_io::set_trace_basics(const std::string &fl,
                                std::shared_ptr<hypercube> d, int reel_h,
                                int head, bool sw, std::shared_ptr<util> p,
                                int i) {
  set_basics(p, d);
...
void trace_io::read_block(int *nw, int *fw, unsigned char *buf, int nbytes) {
  ...
  loaded = true;
  long long to = 0;
  for (int i7 = 0; i7 < nw[7]; i7++) {
    long long s7 = block[7] * (i7 + fw[7]);
    for (int i6 = 0; i6 < nw[6]; i6++) {
      long long s6 = s7 + block[6] * (i6 + fw[6]);
      for (int i5 = 0; i5 < nw[5]; i5++) {
        long long s5 = s6 + block[5] * (i5 + fw[5]);
        for (int i4 = 0; i4 < nw[4]; i4++) {
          long long s4 = s5 + block[4] * (i4 + fw[4]);
          for (int i3 = 0; i3 < nw[3]; i3++) {
            long long s3 = s4 + (long long)(block[3] * (i3 + fw[3]));
            for (int i2 = 0; i2 < nw[2]; i2++) {
              long long s2 =
                  s3 + (long long)(block[2] * (long long)((i2 + fw[2])));
              if (slice) {
                ...
                for (int i1 = 0; i1 < nw[1]; i1++) {
                  long long s1 = s2 + block[1] * (i1 + fw[1]);
