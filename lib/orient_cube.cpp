...
int orient_cube::form_map_name(int iax1, int iax2, int idelta, int *i3,
                               int *i3v) {
  if (order[0] == iax1 && order[1] == iax2)
    *i3 = order[2];
  else if (order[0] == iax2 && order[1] == iax2)
    *i3 = order[2];
  else if (order[0] == iax1 && order[1] == iax2)
    *i3 = order[1];
  else if (order[0] == iax2 && order[1] == iax1)
    *i3 = order[1];
  else {
    *i3 = order[0];
  }
...
void orient_cube::get_viewed_locs(int iax1, int iax2, int *iloc) {
  get_locs(iloc);
  if (shift_ax != -1 && shift_ax != iax1 && shift_ax != iax2) {
    long long hsh;
...
std::shared_ptr<longTensor2D> orient_cube::getIndexMapPtr(int iax1, int iax2,
  ...
  if (found) {
    ...
    std::shared_ptr<longTensor2D> ee= rot_maps[ibig]->getIndexMapPtr();
      auto rot2 = xt::view(ee->mat, xt::all(), xt::all());
      std::cerr<<"found map check "<<rot2(0,0)<<" "<<rot(0,1)<<" "<<rot(0,2)<<std:endl;
      return ee;
...
std::map<long long, int> *orient_cube::get_rot_map(int iax1, int iax2,
                                                   int *n1) {
  int i3a = 0, i3b = 0;
  int ibig = form_map_name(iax1, iax2, 0, &i3a, &i3b);
  update_map_order(ibig, false);
...
void orient_cube::get_true_pos(float *pos, float oversamp) {
  int iloc[8];
  get_locs(iloc);
  long long hsh;
...
void orient_cube::return_grid_loc(float *p, int *iloc, float oversamp) {
  ...
  if (one_shift.size() > 0) {
    ...
    axis a = getAxis(0);
    int look = std::max(
        0, std::min(a.n - 1, (int)((p[0] - a.o) / a.d * oversamp + .5)));
    int i = 0;
    bool found = false;
    while (i < a.n - 1 && !found) {
      if (one_shift[hsh] <= look && one_shift[hsh + 1] >= look) {
        found = true;
        iloc[0] = i;
      } else {
        hsh++;
...
void orient_cube::set_one_shift(int iax, float ov, std::vector<int> buf) {
  ...
  for (long long i2 = 0; i2 < n123 / (long long)n; i2++) {
    for (int i1 = 0; i1 < n; i1++, i++) {
      ...
      begs[i1] = std::min(begs[i1], iam);
      ends[i1] = std::max(ends[i1], iam);
      //       if(i2>8192 && i2 < 8320) fprintf(stderr,"check what I am doing
      //       %d,%d %d %d %d \n",i1,(int)(i2-8192),iam,begs[iam],ends[iam]);
    }
  }
  ...
  //   i1,begs[i1],ends[i1]);
  //   }
  // Now we need to handle the case where some tau doesn't correspond to any
  // samples
  int lastb = -1, laste = -1;
  for (int i = 0; i < n; i++) {
