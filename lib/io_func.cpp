...
#include "iostream"
using namespace SEP;
void io_func::set_basics(std::shared_ptr<util> pars,
...
void io_func::read_block_float(std::vector<int> &nw, std::vector<int> &fw,
                               float *buf) {
  if (nw[0] == 0 && fw[0] == 0 && buf == 0)
    ;
  par->error("Undefined  read_block_float for this type");
}
void io_func::read_block_byte(std::vector<int> &nw, std::vector<int> &fw,
                              unsigned char *buf) {
  if (nw[0] == 0 && fw[0] == 0 && buf == 0)
    ;
  par->error("Undefined read_block_byte for this type");
}
void io_func::convert_to_local(std::vector<int> &nwin, std::vector<int> &fwin,
