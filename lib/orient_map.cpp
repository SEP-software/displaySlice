...
orient_map::orient_map(bool rot, int i_ax1, int i_ax2, int *rotax,
                       std::vector<axis> axrot,
                       std::shared_ptr<intTensor2D> rr1,
                       std::shared_ptr<intTensor2D> rr2, int *begs, int *i_loc,
                       int *ends, int *n_s, bool r1, bool r2,
...
void orient_map::add_pick(const std::string &col, long long index) {
  picks[col].add_pick(index);
}
void orient_map::del_pick(const std::string &col, long long index) {
  picks[col].del_pick(index);
}
void pick_holder::del_pick(long long index) {
  std::vector<long long>::iterator i = hash.begin();
  while (i != hash.end() && *i != index)
    i++;
  if (i == hash.end()) {
    fprintf(stderr,
            "internal error: attempt to delete pick that doesn't exist");
    assert(1 == 2);
  }
  hash.erase(i);
}
...
void orient_map::create_samp_dat_map() {
  samp_exist = true;
  // if(!rotate) return;
  auto mapA = xt::view(map->mat, xt::all(), xt::all());
  ...
  if (1 == 1) {
    for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        if (rev1) {
          if (rev2)
            sample_to_dat[mapA(i2, i1)] = (n2 - i2 - 1) * n1 + n1 - i1 - 1;
          else
            sample_to_dat[mapA(i2, i1)] = i2 * n1 + n1 - i1 - 1;
        } else {
          if (rev2)
            sample_to_dat[mapA(i2, i1)] = i1 + (n2 - i2 - 1) * n1;
          else
            sample_to_dat[mapA(i2, i1)] = i1 + i2 * n1;
        }
      }
    }
  }
  if (1 == 3) {
    ...
    if ((iax1 == rot_ax[0] && iax2 == rot_ax[1]) ||
        (iax1 == rot_ax[1] && iax2 == rot_ax[0])) {
      int ia, ib;
      ...
      for (i2 = 0; i2 < ax_rot[1].n; i2++) {
        ...
        for (i1 = 0; i1 < ax_rot[0].n; i1++) {
          ...
          if (rot2(i2, i1) >= b_2 && rot2(i2, i1) < e_2 &&
              rot1(i2, i1) >= b_1 && rot1(i2, i1) < e_1) {
            add[ib] = rot2(i2, i1) * blks[ib];
            add[ia] = rot1(i2, i1) * blks[ia];
            long long v1 = add[0] + add[1] + add[2] + add[3] + add[4] + add[5] +
                           add[6] + add[7];
            int pos = *i_1 + n1 * (*i_2);
      ...
      int irot2 = iloc[rot_ax[1]], b_1 = b1, e_1 = e1, b_2 = beg[rot_ax[1]],
          e_2 = end[rot_ax[1]];
      for (int ireg2 = b2; ireg2 < e2; ireg2++) {
        for (int i1 = 0; i1 < ax_rot[0].n; i1++) {
          int ireg1 = rot1(irot2, i1);
          int irege = rot2(irot2, i1);
      ...
      int irot1 = iloc[rot_ax[0]], b_1 = b1, e_1 = e1, b_2 = beg[rot_ax[0]],
          e_2 = end[rot_ax[0]];
      for (int ireg2 = b2; ireg2 < e2; ireg2++) {
        for (int i1 = 0; i1 < ax_rot[1].n; i1++) {
          int irege = rot1(i1, irot1);
          int ireg1 = rot2(i1, irot1);
          if (ireg1 >= b_1 && ireg1 < e_1 && irege >= b_2 && irege < e_2) {
            add[iax1] = ireg1 * blks[iax1];
            add[iax2] = ireg2 * blks[iax2];
      ...
      int irot2 = iloc[rot_ax[1]], b_2 = b2, e_2 = e2, b_1 = beg[rot_ax[1]],
          e_1 = end[rot_ax[1]];
      for (int i2 = 0; i2 < ax_rot[0].n; i2++) {
        for (int ireg1 = b1; ireg1 < e1; ireg1++) {
          int ireg2 = rot1(irot2, i2);
          int irege = rot2(irot2, i2);
      ...
      int irot1 = iloc[rot_ax[0]], b_2 = b2, e_2 = e2, b_1 = beg[rot_ax[0]],
          e_1 = end[rot_ax[0]];
      for (int i2 = 0; i2 < ax_rot[1].n; i2++) {
        for (int ireg1 = b1; ireg1 < e1; ireg1++) {
          int irege = rot1(i2, irot1);
          int ireg2 = rot2(i2, irot1);
          if (ireg2 >= b_2 && ireg2 < e_2 && irege >= b_1 && irege < e_1) {
            add[iax1] = ireg1 * blks[iax1];
            add[iax2] = ireg2 * blks[iax2];
            add[rot_ax[0]] = irege * blks[rot_ax[0]];
...
void orient_map::find_loc(int *iloc, int id1, int id2) {
  ...
  if (map_1d.size() > 0) {
    iloc[shift_ax] = map_1d[mapA[id1 + id2 * n1]];
  }
...
void orient_map::formIndexMap() {
  ...
  map = std::make_shared<longTensor2D>(n1, n2);
  auto mapA = xt::view(map->mat, xt::all(), xt::all());
  ...
  if (!rotate) {
    ...
    int b1 = beg[rot_ax[0]], b2 = beg[rot_ax[1]], e1 = end[rot_ax[0]],
        e2 = end[rot_ax[1]];
    ...
    if (ia1 == rot_ax[0] && ia2 == rot_ax[1]) {
      ...
      if (ia1 == iax1) {
        for (int i2 = 0; i2 < n2; i2++) {
          for (int i1 = 0; i1 < n1; i1++, i++) {
            int ib = mm2(i2 * dir2 + f2, i1 * dir1 + f1);
            int ia = mm1(dir2 * i2 + f2, dir1 * i1 + f1);
            ...
            if (ia < b1 || ia >= e1 || ib < b2 || ib >= e2)
              ...
              int i_2 = ib;
              int i_1 = ia;
              mapA(i2, i1) = i_1 * j1 + i_2 * j2 + first;
            }
          }
        }
      } else {
        for (int i2 = 0; i2 < n2; i2++) {
          for (int i1 = 0; i1 < n1; i1++, i++) {
            int ib = mm1(i1 * dir1 + f1, i2 * dir2 + f2);
            int ia = mm1(i1 * dir1 + f1, i2 * dir2 + f2);
          ...
          for (int i2 = 0; i2 < n2; i2++) {
            for (int i1 = 0; i1 < n1; i1++, i++) {
              ...
              if (ia < b1 || ia >= e1 || ib < b2 || ib >= e2) {
                ...
                int i_2 = i2 * dir2 + f2;
                int i_3 = ib;
                mapA(i2, i1) = first + jax1 * i_1 + j2 * i_2 + i_3 * jax2;
              }
            }
          }
        } else {
          ...
          for (int i2 = 0; i2 < n2; i2++) {
            for (int i1 = 0; i1 < n1; i1++, i++) {
              ...
              if (ia < b1 || ia >= e1 || ib < b2 || ib >= e2)
                ...
                int i_2 = i2 * dir2 + f2;
                int i_3 = ia;
                mapA(i2, i1) = first + jax2 * i_1 + i_2 * j2 + i_3 * jax1;
              }
            }
          }
        }
          ...
          for (int i2 = 0; i2 < n2; i2++) {
            for (int i1 = 0; i1 < n1; i1++, i++) {
              ...
              if (ia < b1 || ia >= e1 || ib < b2 || ib >= e2)
                ...
                int i_1 = i1 * dir1 + f1;
                int i_3 = ib;
                mapA(i2, i1) = first + i_1 * j1 + i_2 * jax1 + i_3 * jax2;
              }
            }
          }
        } else {
          ...
          for (int i2 = 0; i2 < n2; i2++) {
            for (int i1 = 0; i1 < n1; i1++, i++) {
              ...
              if (ia < b1 || ia >= e1 || ib < b2 || ib >= e2)
                ...
                int i_1 = i1 * dir1 + f1;
                int i_3 = ia;
                mapA(i2, i1) = first + i_1 * j1 + i_2 * jax2 + i_3 * jax1;
              }
            }
          }
        }
    ...
    for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        if (mapA(i2, i1) < 0)
          ...
          int ilook = mapA(i2, i1) % ns[0];
          if (i % nfast == 0)
            iold = 0;
          long long ibase, ib = mapA(i2, i1) - (long long)ilook;
          ibase = ib;
          bool found = false;
          int is = 0;
          if (fast) {
            ...
            while (!found && itest <= std::min(iold + 1, ns[0] - 2)) {
              if (map_1d[ibase + itest] <= ilook &&
                  map_1d[ibase + itest + 1] >= ilook) {
                found = true;
              } else {
                itest++;
              }
...
void orient_map::rotation_to_grid_loc(int *loc) {
  ...
  if (iax1 != 0 && iax2 != 0)
    return;
  long long ib = loc[1] * skip[1] + loc[2] * skip[2] + loc[3] * skip[3] +
                 loc[4] * skip[4] + loc[5] * skip[5] + loc[6] * skip[7] +
                 loc[7] * skip[7];
...
void orient_map::orient_data_loc(int *cur, int *ipos) {
  if (map_1d.size() > 0) {
    ...
    if (iax1 == 0 || iax2 == 0 || 0 == 0)
      ipos[shift_ax] = map_1d[mapA(i2, i1)];
    else {
      // Figure out our current position in the flattened space
      int ifind = cur[0]; // Current position in un-flattend space
      long long ibef =
          mapA(i2, i1) - cur[0]; // The location in unflattened space at z=0
    }
  }
}
void orient_map::shift_data_image(float *pct) {
  if (shift_ax < 0 || (shift_ax != iax1 && shift_ax != iax2))
    return;
  ...
  if (iax1 == shift_ax) {
    int i1 = 0, i2 = (int)((float)(n2 - 1) * pct[1]);

    while (i1 < n1 && mapA(i2, i1) < 0)
      i1++;
    if (i1 < n1) {
      hsh = mapA(i2, i1);
    ...
    int i2 = 0, i1 = (int)((float)(n1 - 1) * pct[0]);

    while (mapA(i2, i1) < -1 && i2 < n2)
      i2 += 1;
    if (i2 < n2) {
      hsh = (long long)(mapA(i2, i1) / (long long)ns[0]) * (long long)ns[0] +
  ...
  if (shift_ax == iax1) {
    ...
    while (mapA(i2, ibeg) < 0 && ibeg < n1 - 1)
      ibeg++;
    if (ibeg == n1)
      return;
    int iend = n1 - 1;
    while (mapA(i2, iend) < 0 && iend > 0)
      iend--;
    int bef = map_1d[mapA(i2, ibeg)];
    int enf = map_1d[mapA(i2, iend)];
    ...
    if (bef < enf) {
      float ilook =
          pct[iax1] * n1 + beg[iax1]; // This is the value we are looking for
      if (bef >= ilook) { // If the value is before what we have in our map
                          // (shouldn't happen) return boundary
        pct[iuse] = .01;
        return;
      }
    ...
    // Map in negative direction
      ...
      for (int i = iend - 1; i >= ibeg; i--) {
        int beg = map_1d[mapA(i2, i1)];
        if (ilook >= enf && ilook < beg) {
          pct[0] = (float)((ilook - enf) / (float)(beg - enf) +
                           (float)(i - i2 * n1)) /
                   (float)n1;
          return;
        }
  ...
  for (int i = 1; i < n2; i++) {
    ...
    int end = map_1d[mapA(i, i1)];
    if (ilook >= bef && ilook < end) {
      pct[1] =
          (float)((ilook - bef) / (float)(end - bef) + (float)i) / (float)n2;
...
void orient_map::calc_get_pars(int iax1, int iax2, long long *j1, long long *j2,
                               long long *first) {
  *first = 0;
  ...
  for (int i = 0; i < 8; i++) {
    if (i > 0)
      ipos = ipos + skip[i] * iloc[i];
    if (i == iax1)
      *j1 = skip[i];
    else if (i == iax2)
      *j2 = skip[i];
    else {
      *first = *first + skip[i] * iloc[i];
    }
  }
      ...
      if (map_1d[ipos] >= iloc[0])
        ...
        while (i < n1 - 1 && !found) {
          if (map_1d[ipos + i] > iloc[0]) {
            found = true;
            if (map_1d[ipos + i] - iloc[0] < iloc[0] - iold)
              iuse = i;
            else
              iuse = i - 1;
          }
