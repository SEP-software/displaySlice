  ...
  if (1 == 1) {
    for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        if (rev1) {
          if (rev2)
            sample_to_dat[map[i1 + i2 * n1]] = (n2 - i2 - 1) * n1 + n1 - i1 - 1;
          else
            sample_to_dat[map[i1 + i2 * n1]] = i2 * n1 + n1 - i1 - 1;
        } else {
          if (rev2)
            sample_to_dat[map[i1 + i2 * n1]] = i1 + (n2 - i2 - 1) * n1;
          else
            sample_to_dat[map[i1 + i2 * n1]] = i1 + i2 * n1;
        }
      }
    }
  }
    ...
    if (iax1 == 0 || iax2 == 0 || 0 == 0)
      ...
      /*
         ...
         while(i < ns[0]){
           iend=map_1d[ibeg+i];
           if(iend > ifind){
              fprintf(stderr,"returning in the middle %d %d %d
      %d\n",i,ibef,iend,ifind); if(iend-ifind < ifind-ibef) ipos[0]=iend; else
      ipos[0]=ibef; return;
           }
           ibef=iend;
           i++;
