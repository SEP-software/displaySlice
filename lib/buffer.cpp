              ...
              for (int i2 = 0; i2 < nuse[2]; i2++) {
                ...
                for (int i1 = 0; i1 < nuse[1]; i1++) {
                  ...
                  for (int i0 = 0; i0 < nuse[0]; i0++) {
                    iin = shift[1] + ii[0][i0];
                    nput += 1;
                    memcpy((bufout + (iout + nput) * (long long)ssz),
                           (bufin + iin * (long long)ssz), ssz);
                  }
                }
              }
            }
...
buffer::gridToIndex(std::shared_ptr<longTensor2D> ind) {
  ...
  if (resamp_1 && 3 == 3) {
    long long nin = hyper_buf->getAxis(1).n;
    long long nout = skip[1];

    for (long long i2 = 0, i = 0; i2 < ns[1]; i2++) {
      for (long long i1 = 0; i1 < ns[0]; i1++, i++) {
        if (inA(i2, i1) == -1)
          outA(i2, i1) = -1;
        else
          outA(i2, i1) = (long long)i2 * nout + ii[0][i1];
      }
    }
      return out;
    ...
    kip[0] = 1;
    for (int i = 0; i < 7; i++) {
      kip[i + 1] = kip[i] * hyper_buf->getAxis(i).n;
    }

    for (long long ii2 = 0; ii2 < ns[1]; ii2++) {
      for (long long ii1 = 0; ii1 < ns[0]; ii1++) {
        long long v = inA(ii2, ii1);
        int i7 = ii[7][(int)(v / kip[7])];
        v = v - kip[7] * (int)(v / kip[7]);
        int i6 = ii[6][(int)(v / kip[6])];
        v = v - kip[6] * (int)(v / kip[6]);
        int i5 = ii[5][(int)(v / kip[5])];
        v = v - kip[5] * (int)(v / kip[5]);
        int i4 = ii[4][(int)(v / kip[4])];
        v = v - kip[4] * (int)(v / kip[4]);
        int i3 = ii[3][(int)(v / kip[3])];
        v = v - kip[3] * (int)(v / kip[3]);

        int i2 = ii[2][(int)(v / kip[2])];
        v = v - kip[2] * (int)(v / kip[2]);
        int i1 = ii[1][(int)(v / kip[1])];
        int i0 = ii[0][(int)(v - kip[1] * (int)(v / kip[1]))];

        outA(ii2, ii1) = i0 + skip[1] * i1 + skip[2] * i2 + i3 * skip[3] +
                       i4 * skip[4] + i5 * skip[5] + skip[6] * i6 +
                       i7 * skip[7];
      }
    }
    return out;
  ...
  std::shared_ptr<longTensor2D> buffer::formIndexMap(
      std::shared_ptr<orient_cube> pos, int iax1, int iax2, int f1, int e1,
      int f2, int e2) {
    std::shared_ptr<longTensor2D> bb =
        pos->getIndexMapPtr(iax1, iax2, f1, e1, f2, e2, 0);

     std::shared_ptr<longTensor2D> in=bb->clone();
     if(same) return in;
    ...
    std::shared_ptr<longTensor2D> out=in->clone();
      auto inA = xt::view(in->mat, xt::all(), xt::all());
  auto outA = xt::view(out->mat, xt::all(), xt::all());
    if (resamp_1 && 3 == 3) {
      int nin = pos->getAxis(0).n;
      int nout = skip[1];
      for (int ii2 = 0; ii2 < n2; ii2++) {
        for(int ii1=0; ii1 <n1; ii1++){
        if (inA(ii2,ii1) == -1)
          outA(ii2,ii1) = -1;
        else {
          outA(ii2,ii1) = ii2 * nout + ii[0][ii1];
        }
      }
      }
    ...
    for (int ii2 = 0; ii2 < n2; ii2++) {
      ...
      outA(ii2,ii1)= i0 + skip[1] * i1 + skip[2] * i2 + i3 * skip[3] + i4 * skip[4] +
  ...
  /*
  long long *buffer::formIndexMap(std::shared_ptr<orient_cube>pos, int iax1,
   ...
   */
  void buffer::calc_get_pars(std::shared_ptr<orient_cube> pos, int iax1,
                             int iax2, long long *j1, long long *j2,
                             long long *first) {
    *first = 0;
    for (int i = 0; i < 8; i++) {
      if (i == iax1)
        ...
        *j2 = skip[i];
        ...
        if (hold[i]) {
          if (same) {
            *first = *first + skip[i] * pos->get_loc(i);

          } else {
            *first = *first + skip[i] * ii[i][pos->get_loc(i)];
          }
        }
      }
    }
  ...
  void buffer::window_to_local(std::vector<int> nwview,
                               std::vector<int> fwview) {
    n123_view = 1;
    n123_buf = 1;
    long long j = 1;
    for (int i = 0; i < 8; i++) {
      int nwio = hyper_io->getAxis(i + 1).n;
  ...
  bool buffer::hold_point(float *pos_loc, bool *use_axis) {
    ...
    for (int i = 0; i < 8; i++) {
      if (!hold[i] && pos_loc[i] != loc[i] && use_axis[i] &&
        ...
        //       "<<pos_loc[i]<<"!="<<loc[i]<<" "<<use_axis[i]<<std::endl;
        // std::cout << "FAILING "<<hold[i]<<" "<<pos->get_loc(i)<<"
        // "<<loc[i]<<" " << use_axis[i]<<endl;
        return false;
      }
    }
    return true;
  }
  std::vector<float> buffer::returnHisto() {
    std::vector<float> tmp(256);
    memcpy((void *)tmp.data(), (const void *)histo, 256 * sizeof(float));
    return tmp;
  }
