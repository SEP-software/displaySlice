...
std::map<int, std::shared_ptr<picks_vec>> pick_bucket::return_picks_from_range(
  ...
  for (std::vector<int>::iterator i = keys.begin(); i != keys.end(); i++) {
    for (int i2 = 0; i2 < n2; i2++) {
      int buck_use = buck_base + (int)((*i / BUCKET_SIZE)) * block_b[ip] +
                     i2 * block_b[iax2];
...
std::shared_ptr<picks_vec> pick_bucket::return_pick_plane(
    int *iloc, const std::string col) {
...
std::shared_ptr<pick_new> pick_bucket::add_pick(int *iloc, long long p, int te,
                                                const std::string &col, int ex,
                                                const std::string &t) {
  std::shared_ptr<pick_new> pp =
      buckets[bucket_num(p)]->add_pick(iloc, p, te, col, ex, t);
  return pp;
}
...
std::shared_ptr<pick_new> pick_bucket::get_pick(long long p,
                                                const std::string &col) {
  return buckets[bucket_num(p)]->get_pick(p, col);
}
void pick_bucket::del_pick(long long p, const std::string &col) {
  buckets[bucket_num(p)]->del_pick(p, col);
}
long long pick_bucket::find_nearest(long long p, int iax1, int iax2,
                                    const std::string &col) {
  int index[8], send[8];
  get_index(p, index);
  std::vector<int> bs;
  ...
  int b = BUCKET_SIZE;
  send[iax1] = std::max(0, index[iax1] - b);
  send[iax2] = std::max(index[iax2] - b, 0);
  bs.push_back(bucket_num(send));
  send[iax2] = index[iax2];
  bs.push_back(bucket_num(send));
  send[iax2] = std::min(next[iax2], index[iax2] + b);
  bs.push_back(bucket_num(send));
  send[iax1] = index[iax1];
  bs.push_back(bucket_num(send));
  send[iax1] = std::min(next[iax1], index[iax1] + b);
  bs.push_back(bucket_num(send));
  send[iax2] = index[iax2];
  bs.push_back(bucket_num(send));
  send[iax2] = std::max(0, index[iax2] - b);
  bs.push_back(bucket_num(send));
  send[iax1] = index[iax1];
  bs.push_back(bucket_num(send));
  ...
  for (int i = 0; i < (int)picks->return_size(); i++) {
    ...
    if (valid) {
      int dist =
          (myp->get_loc(iax1) - loc[iax1]) * (myp->get_loc(iax1) - loc[iax1]) +
...
std::shared_ptr<picks_vec> pick_bucket::picks_from_vec(std::vector<int> *bs,
                                                       const std::string &col) {
...
void pick_bucket::delete_bucket() { buckets.clear(); }
std::shared_ptr<picks_vec> pick_bucket::get_pick_type(int typ,
                                                      const std::string &col) {
  std::shared_ptr<picks_vec> pk(new picks_vec());
  for (int i = 0; i < (int)buckets.size(); i++)
    buckets[i]->get_parse_picks(pk, col, typ);
  return pk;
}
std::map<long long, std::shared_ptr<pick_new>> pick_bucket::return_all(
    const std::string &col) {
...
std::shared_ptr<picks_vec> pick_bucket::return_all_picks(
    const std::string &col) {
