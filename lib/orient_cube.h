...
class orient_cube : public position {
 public:
  orient_cube();
  orient_cube(std::shared_ptr<position> p, std::vector<int> o,
              std::shared_ptr<orientation_server> s = nullptr);
  orient_cube(std::shared_ptr<orient_cube> ori);
  orient_cube(std::shared_ptr<SEP::hypercube> h,
              std::shared_ptr<orientation_server> s = nullptr);
  ...
  void set_reverse(int iax, bool val) {
    this->reverse[iax] = val;
    rotation_change();
  }
  void flip_axis(int iax) {
    if (this->reverse[order[iax]])
      this->reverse[order[iax]] = false;
    else
      this->reverse[order[iax]] = true;
    rotation_change();
  }
  ...
  void set_rot_angle(float a) {
    ang = a / 45. * atan(1.);
    set_rotation();
  }
  void set_rotation();
  void set_rot_pt(int *pt) {
    memcpy(rot_pt, pt, 8 * sizeof(int));
    set_rotation();
  }
  void get_rot_pt(int *pt) { memcpy(pt, rot_pt, 8 * sizeof(int)); }
  void set_no_rotate();
  void set_no_shift();
  void get_rot_axes(int *a1, int *a2) {
    *a1 = rot_ax[0];
    *a2 = rot_ax[1];
  }
  ...
  std::vector<int> return_picks_index(int iax1, int iax2, int idelta,
                                      const std::string &col) {
    int i3a = 0, i3b = 0;
    return rot_maps[form_map_name(iax1, iax2, idelta, &i3a, &i3b)]
        ->return_picks_index(col);
  }

  void set_picks(int iax1, int iax2, int idelta,
                 std::shared_ptr<picks_vec> pks) {
    int i3a = 0, i3b = 0;
    rot_maps[form_map_name(iax1, iax2, idelta, &i3a, &i3b)]->set_picks(pks);
  }
  void add_pick(int iax1, int iax2, int idelta, const std::string &col,
                long long index) {
    int i3a = 0, i3b = 0;

    rot_maps[form_map_name(iax1, iax2, idelta, &i3a, &i3b)]->add_pick(col,
                                                                      index);
  }
  void del_pick(int iax1, int iax2, int idelta, const std::string &col,
                long long index) {
    int i3a = 0, i3b = 0;

    rot_maps[form_map_name(iax1, iax2, idelta, &i3a, &i3b)]->del_pick(col,
                                                                      index);
  }
  ...
  inline void rotate_pt(float x1, float x2, float *y1, float *y2) {
    *y1 = (x1 - rot_cen[0]) * cos(ang) + sin(ang) * (x2 - rot_cen[1]) +
          rot_cen[0];
    *y2 = -(x1 - rot_cen[0]) * sin(ang) + cos(ang) * (x2 - rot_cen[1]) +
          rot_cen[1];
  }
  void convert_picks(std::shared_ptr<picks_vec> in,
                     std::shared_ptr<picks_vec> out) {
    for (int i = 0; i < in->return_size(); i++)
      out->add_pick(convert_pick(in->return_pick(i)));
  }
  inline std::shared_ptr<pick_new> convert_pick(std::shared_ptr<pick_new> in) {
    ...
    float xin = in->iloc[rot_ax[0]] * a1.d + a1.o,
          yin = a2.o + a2.d * in->iloc[rot_ax[1]], xout, yout;
    rotate_pt(xin, yin, &xout, &yout);
    std::shared_ptr<pick_new> out = in->clone();

    out->iloc[rot_ax[0]] =
        std::max(0, std::min(ax_rot[0].n - 1,
                             (int)(0.5 + (xout - ax_rot[0].o) / ax_rot[0].d)));
    out->iloc[rot_ax[1]] =
        std::max(0, std::min(ax_rot[1].n - 1,
                             (int)(0.5 + (yout - ax_rot[1].o) / ax_rot[1].d)));
  ...
  void rotate_loc(int *iloc) {
    SEP::axis a1 = getAxis(rot_ax[0]), a2 = getAxis(rot_ax[1]);
    float xin = iloc[rot_ax[0]] * a1.d + a1.o,
          yin = a2.o + a2.d * iloc[rot_ax[1]], xout, yout;
    rotate_pt(xin, yin, &xout, &yout);
    iloc[rot_ax[0]] =
        std::max(0, std::min(ax_rot[0].n - 1,
                             (int)(0.5 + (xout - ax_rot[0].o) / ax_rot[0].d)));
...
class orients {
 public:
  ...
  std::shared_ptr<orient_cube> return_active() { return my_or[active_pan]; }
