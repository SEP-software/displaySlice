...
float_buffer::float_buffer(std::shared_ptr<paramObj> p,
  ...
  calc_read_loop(nwbuf, fwbuf.data(), nwio, fwio, nloop, ndim);
  read_buffer(nwbuf, fwbuf.data(), nwio, fwio, ndim, nloop);
  float minv, maxv;
          ...
          for (int i3 = 0; i3 < nloop[3]; i3++) {
            ...
            for (int i2 = 0; i2 < nloop[2]; i2++) {
              ...
              for (int i1 = 0; i1 < nloop[1]; i1++) {
                ...
                //   off,(unsigned char*)tbuf,(unsigned
                //   char*)fbuf,(int)sizeof(float));
                off += nblock;
                count += 1;
              }
            }
...
float_buffer::getCharData(std::shared_ptr<longTensor2D> index) {
  ...
  std::vector<int> ns = index->getNs();
  std::shared_ptr<byteTensor2D> out = mk_shared<byteTensor2D>(ns[0], ns[1]);
...
std::shared_ptr<floatTensor2D> float_buffer::get_float_data(std::shared_ptr<orient_cube> pos, int iax1,
  ...
  lstd::shared_ptr<longTensor2D> index =
      formIndexMap(pos, iax1, iax2, f1, e1, f2, e2);
                auto indexA = xt::view(index->mat, xt::all(), xt::all());

  std::vector<int> ns=index->getNs();
  std::shared_ptr<floatTensor2d> out=std::make_shared(ns[0],ns[1]);
  float minv, maxv;
  io->return_clips(&minv, &maxv);
            auto outA = xt::view(out->mat, xt::all(), xt::all());
