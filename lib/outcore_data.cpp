...
using namespace SEP;
void outcore_data::hold_pattern(std::shared_ptr<orient_cube> pos, int iax1,
                                int iax2, bool *hold) {
  for (int i = 0; i < 8; i++) hold[i] = false;
  int order[8];
  pos->get_orders(order);
  hold[iax1] = hold[iax2] = true;
  if (iax1 != order[0] && iax2 != order[0])
    hold[order[0]] = true;
  else if (iax1 != order[1] && iax2 != order[1])
    hold[order[1]] = true;
  else
    hold[order[2]] = true;
}
void outcore_data::delete_dataset(std::shared_ptr<orient_cube> pos, int iax1,
                                  int iax2) {
  bool hold[8], hh[8];
  ...
  while (i < (int)buf.size() && !found) {
    ...
    for (int j = 0; j < 8; j++)
      if (hold[j] != hh[j] && axes[j].n > 1) test = false;
...
outcore_data_float::outcore_data_float(std::string title, const std::string &nm,
...
std::shared_ptr<buffer> outcore_data_float::create_buffer(
...
outcore_data_byte::outcore_data_byte(std::string title, const std::string &nm,
                                     std::shared_ptr<hypercube> g,
                                     std::shared_ptr<io_func> i,
                                     std::shared_ptr<paramObj> p, int in,
                                     int im) {
...
std::shared_ptr<buffer> outcore_data_byte::create_buffer(
    std::shared_ptr<orient_cube> pos, int iax1, int iax2) {
